...
apiTest.for([
	...
	async ([_, withOptionals], {api: {Initiative, PdfAsset, ImageAsset}}) => {
		...
		expect(initiativeCopy.toJSON()).toEqual({
			...
			website: withOptionals ? 'https://google.com/' : undefined,
			pdf: pdfAsset.name,
			image: imageAsset?.name ?? undefined,
			deadline: withOptionals ? '2025-05-12' : undefined,
			initiatedDate: withOptionals ? '2024-01-05' : undefined,
			bundeskanzleiUrl: withOptionals ? 'https://www.bk.admin.ch' : undefined,
			organisations: [],
			signatures: [],
